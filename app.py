...
def hello(mydata):
    crime_url_template ='https://data.police.uk/api/crimes-street/all-crime?lat={lat}&lng={lng}&date={data}'#api enterance
    my_latitude = '51.52369'               #set the parameters for api
    ...
    categories = {categ["url"]:categ["name"] for categ in categories_json}#build a dictionary from the data
    ...
    for key in crime_category_stats:#insert the name and the number for each category into the database
